...
Cell::Cell(const Cell::CreateNewCell *parameters, const unsigned int actorId_, ActorFramework::ExecutionUnit *executionUnit_)
...
void Cell::processMessage(const int messageSize, const ActorFramework::Message::MessageData *messageData) {
    switch (messageData->type) {
        ...
        case SQUIRREL_SIMULATION_MESSAGES::NEW_MONTH: {
            ...
            break;
        default:
            logPrint(ActorFramework::Logger::LogLevel::ERROR, "ERROR: Cells: Should not receive messages of type: " + ActorFramework::Logger::numToString(messageData->type));
            break;
    }
}
void Cell::update() { logPrint(ActorFramework::Logger::LogLevel::ERROR, "ERROR: Cells: Should not be updated"); }

ActorFramework::Actor *Cell::createNewActor(const unsigned int actorId_, const char *messageData) {
