...
Squirrel::Squirrel(const Squirrel::SquirrelBasicData* squirrelBasicData_,
                   const unsigned int actorId_, ActorFramework::ExecutionUnit* executionUnit_)
    ...
    if (actorId_ != 0) {
        ...
        logPrint(ActorFramework::Logger::LogLevel::EXHAUSTIVE, "Squirrel born at: [ " + std::to_string(squirrelBasicData.positionX) + ", " + std::to_string(squirrelBasicData.positionY) + " ] " + std::string(*this));
    }
}

void Squirrel::processMessage(const int messageSize, const ActorFramework::Message::MessageData* messageData) {
    if (messageData->type == SQUIRREL_SIMULATION_MESSAGES::CELL_INFECTION) {
        ...
        logPrint(ActorFramework::Logger::LogLevel::ERROR, "ERROR: Squirrels: Should not receive messages of type: " + ActorFramework::Logger::numToString(messageData->type));
    }
}
...
ActorFramework::Actor* Squirrel::createNewActor(const unsigned int actorId_, const char* messageData) {
...
void Squirrel::checkGiveBirth() {
    if (stepsAlive > 0 && (stepsAlive % STEPS_STORED_DATA) == 0 && willGiveBirth()) {
        ...
        ActorFramework::Message message(sizeof(SquirrelBasicData), ActorFramework::Message::CREATE_NEW_ACTOR);
...
Squirrel::operator std::string() const {
    return "{ Id: " + ActorFramework::Logger::numToString(actorId) + (squirrelBasicData.isInfected ? " is infected" : " is healthy") +
           " Num. steps alive: " + ActorFramework::Logger::numToString(stepsAlive) +
           " Num. steps infected: " + ActorFramework::Logger::numToString(stepsInfected) +
           " Num children: " + ActorFramework::Logger::numToString(numChildren) + " }";
}
...
void Squirrel::checkToInfect() {
    if (squirrelBasicData.isInfected) {
        ...
        if (squirrelBasicData.isInfected) {
            ...
            logPrint(ActorFramework::Logger::LogLevel::EXHAUSTIVE, "Squirrel Infected: " + std::string(*this));
...
bool Squirrel::checkToKill() {
    if (stepsInfected > STEPS_STORED_DATA && willDie()) {
        ...
        logPrint(ActorFramework::Logger::LogLevel::EXHAUSTIVE, "Squirrel Killed: " + std::string(*this));
        return true;
    } else {
        return false;
    }
