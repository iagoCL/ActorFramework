...
namespace ActorFramework {
    ...
    Message::Message(int messageSize_, int tag_) : messageSize(messageSize_ + MESSAGE_DATA_HEADER_SIZE), tag(tag_) {}
    Message::Message(unsigned int origActorId, unsigned int destActorId, unsigned int type, int messageSize_, int tag_)
        : messageSize(messageSize_ + MESSAGE_DATA_HEADER_SIZE), tag(tag_) {
